...
const { getEvmExplorerUrl } = require('./evm-config.ts')
...
async function processPayoutServerOnly(args) {
  ...
  if (supabaseUrl && supabaseServiceKey) {
    try {
      ...
      await supabase.from('transactions').insert({
        ...
        amount: poolBnb * houseCutPercentage,
        related_entity_id: matchId,
      ...
      if (matchId) {
        await supabase.from('matches').update({
          ...
          metadata: {
            ...
            payout_amount: poolBnb * (1 - houseCutPercentage),
            house_cut_amount: poolBnb * houseCutPercentage,
          },
        }).eq('id', matchId)
      }
    } catch (e) {
  ...
  console.log('✅ payout_executed', {
    ...
    winner: winnerAddress,
    amount: poolBnb * (1 - houseCutPercentage),
    houseAmount: poolBnb * houseCutPercentage,
    escrow: from?.id,
    winnerSignature,
    houseSignature,
  })
